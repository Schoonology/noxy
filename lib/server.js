...
function NoxyServer(options) {
  ...
  self.port = parseInt(process.env.PORT, 10) || self.port || options.port || options.tunnel.port
...
function _privateSocketConnection(socket) {
  ...
  if (self._tunnel) {
    console.log('Got private connection too late.')
    socket.destroy()
    return
  }
  ...
  socket.once('data', function checkPasscode(data) {
    if (self.passcode.toString() !== msgpack.unpack(data).passcode.toString()) {
      console.log('Failed.')
      socket.destroy()
      self._tunnel = null
      return
...
function _publicSocketConnection(socket) {
  ...
  if (!self._tunnel) {
    console.log('Got public connection too soon.')
    socket.destroy()
    return
  }
